...
为解决以上问题，给调用者提供良好的智能合约调用体验，Simplechain提出**CNS合约命名服务**。
...
ENS (Ethereum Name Service) ，Simplechain名称服务。

ENS的功能类似我们较熟悉的DNS(Domain Name Service)域名系统，但提供的不是Internet网址，而是将Simplechain(Ethereum)合约地址和钱包地址以xxxxxx.eth网址的方式表示，用于存取合约或转账。两者相比：
...
- 调用合约时，SDK引入合约的Java类，并加载实例化。load加载接口可传入合约地址（原有Simplechain方式）或合约名称和合约版本的组合（CNS方式），SDK处理CNS方式时通过调用CNS模块查询链上信息来获取合约地址。
...
**预编译合约**是Simplechain底层通过C++实现的一种高效智能合约，用于Simplechain底层的系统信息配置与管理。引入precompiled逻辑后，Simplechain节点执行交易的流程如下：
